  ...
  async function loadPost(post) {
    ...
    if (post.repost) {
      ...
      if (res.ok) {
        ...
        retweetPostTime.innerText = new Date(repostInfo.time).toLocaleString();
      ...
      let retweetContent = document.createElement("div");
      retweetContent.className = "retweetcontent";
      retweetContent.innerText = "The post that was reposted has been deleted.";
      postContent.appendChild(retweetContent);
    }
...
${postActionsInsert}
  ...
  if (homePosts) {
    Promise.all(homePosts.posts.map((post) => loadPost(post))).catch((e) => {
      console.error(e);
    });
  }
});
